...
- `npm run build` — типизация через `tsc --noEmit` и сборка продакшен-бандла
- `npm run build:prerender` — типизация через `tsc --noEmit`, сборка клиентского и SSR-бандла с последующим статическим рендером в `dist/index.html`
